...
def validate_experiment_outputs(
    ...
    *,
    ...
    required_columns = {"record_id", "exposure", actual_col, predicted_col}
    ...
    predicted = frame[predicted_col].astype(float).to_numpy()
    actual = frame[actual_col].astype(float).to_numpy()
...
def _lift_summary(
    ...
    *,
    ...
    merged = champion[["record_id", actual_col, "exposure", champion_predicted_col]].merge(
    ...
    champion_panel = full_metric_panel(
        merged[actual_merged_col],
        merged[champion_merged_col],
        merged["exposure"],
        tweedie_power=tweedie_power,
        target_mode=target_mode,
    )
