...
def render_handoff_markdown(config: ProjectConfig, context: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "  steps, not a single jump to a complex method.",
            "- Prefer cheap, interpretable changes first: a feature transformation, an interaction term,",
            "  a one-feature GLM, a modest hyperparameter shift around the current champion.",
            "- Reach for higher-capacity models (GBM, deep trees, ensembles) only after the simpler",
            "  paths have plateaued. A GBM in cycle 1 is almost always premature.",
            "- One change at a time — each proposal should be readable as 'X relative to the champion'.",
            "- Explore broadly: try several different cheap ideas before doubling down on any one.",
            "",
            "## Fixed Constraints",
...
def render_proposal_instructions(config: ProjectConfig, context: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "- Strongly prefer cheap, interpretable steps: a single feature transformation, an",
            "  interaction, a low-capacity GLM, or a modest hyperparameter shift on the current",
            "  champion.",
            "- Reserve higher-capacity methods (GBM, deeper trees, ensembling) for after the simple",
            "  paths have been explored. Proposing a GBM in the first few cycles is almost always",
            "  premature.",
            "- One change at a time. Your `change_summary` should read as 'X relative to the",
            "  champion'.",
            "- Across cycles, prioritise breadth — try several different cheap ideas before iterating",
            "  deeply on any one.",
            "",
            "## Fixed constraints",
