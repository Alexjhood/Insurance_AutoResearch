...
export function Journey(){const {orchId}=useParams();const q=useSnapshot(orchId);const s=q.data;
  ...
  if(!s)return <PageState loading={q.isLoading} error={q.error}/>;return <main className="page journey"><header className="journey-heading"><span className="eyebrow">Campaign narrative</span><h1>The Journey</h1><p>From global mean to consolidated champion, with every decision linked to its evidence.</p></header><MissionTimeline delegations={s.delegations} experiments={s.experiments} notes={s.notes} activeDelegationId={activeChapter}/><div className="narrative"><section className="launch panel"><Badge kind="baseline">Launch</Badge><h2>{s.campaign.dataset} / {s.campaign.target_mode}</h2><p>{s.delegations[0]?.brief.direction}</p><div className="chips">{s.delegations[0]?.brief.starting_knowledge.map(x=><span key={x}>{x}</span>)}</div></section>{s.delegations.map((d,di)=>{const experiments=s.experiments.filter(e=>e.delegation_id===d.delegation_id);const notes=s.notes.filter(n=>n.delegation_id===d.delegation_id||(!n.delegation_id&&di>0&&di<s.delegations.length));return <div key={d.delegation_id}>{di>0&&notes.filter(n=>n.kind==='reflection').map((n,i)=><aside className="interlude panel" key={`${n.at}-${i}`}><Badge kind="reflection">Orchestrator reflection</Badge><p>{n.text}</p></aside>)}{notes.filter(n=>n.kind==='takeover').map((n,i)=><aside className="takeover-scene panel" key={`${n.at}-${i}`}><Badge kind="takeover">Takeover · {d.delegation_id}</Badge><h2>Mission recovery</h2><p>{n.text}</p></aside>)}<section className="chapter" id={`chapter-${d.delegation_id}`}><header className="chapter-header"><span className="chapter-number">0{di+1}</span><div><span className="eyebrow">Delegation chapter</span><h2>{d.delegation_id} · {d.brief.name??d.backend}</h2><p>{d.brief.direction}</p></div><Link to={`/o/${orchId}/delegations/${d.delegation_id}`}>Forensic view →</Link></header><div className="brief-card panel"><b>Mission brief</b><div className="chips">{d.brief.constraints.map(c=><span key={c}>✓ {c}</span>)}</div>{d.brief.seed_champion&&<p className="mono">Seed: {d.brief.seed_champion.experiment_id}</p>}</div>{experiments.filter(e=>!e.is_seed&&!e.is_baseline).map((e,i)=><ExperimentCard key={e.experiment_id} e={e} orchId={orchId!} forfeited={!e.comparison?.decision&&(d.delegation_id==='d02'||i>=d.budget.decided)}/>)}{d.distress.active.map(flag=><aside className="distress-callout" key={flag}><Badge kind="distress">{flag}</Badge><p>{d.distress.detail}</p></aside>)}</section></div>})}<section className="finale panel" id="finale"><Badge kind="promote">Finale</Badge><h2>Playoff consolidation</h2><div className="finalist-grid">{s.playoff?.finalists.map(f=><div key={f.delegation_id}><b>{f.delegation_id}</b><MetricNumber value={f.gini_weighted}/><span>{f.model_family}</span></div>)}</div>{s.playoff?.final&&<p className="mono">Champion: {s.playoff.final.source_experiment_id}</p>}<Link to={`/o/${orchId}#playoff`}>Open playoff panel →</Link></section></div></main>}
