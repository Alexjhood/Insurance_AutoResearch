...
export function Journey() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(entries => entries.forEach(entry => entry.isIntersecting && setActiveChapter(entry.target.id.replace('chapter-', ''))), { rootMargin: '-35% 0px -55% 0px' });
  ...
  return <main className="page journey">
    ...
    <div className="narrative">
      <section className="launch panel"><Badge kind="baseline">Launch</Badge><h2>{s.campaign.dataset} / {s.campaign.target_mode}</h2>{plan && <blockquote><b>Orchestrator plan</b><br />{plan.text}</blockquote>}<p>{s.delegations[0]?.brief.direction}</p><div className="chips">{s.delegations[0]?.brief.starting_knowledge.map(x => <span key={x}>{x}</span>)}</div></section>
