...
export interface OrchestratorIdentity { provider: string; model: string; effort: string | null }
export interface IndexEntry {
  orch_id: string; alias: string | null; dataset: string; target_mode: string; status: string; created_at: string;
  ended_at: string | null; orchestrator_model: string; orchestrator: OrchestratorIdentity; stale: boolean; backends: string[]; n_delegations: number;
  cycles_committed: number; cycles_used: number; cycles_attempted: number; seed_evals: number; cycles_forfeited: number; final_gini: number | null;
  baseline_gini: number | null; total_tokens: TokenTotals; cache_hit_rate: number | null;
  wall_clock_minutes: number | null; distress_count: number; takeover_count: number; champion_spark: number[]; cost_usd: number | null; cost_estimated: boolean;
}
...
export interface Campaign { orch_id: string; dataset: string; target_mode: string; status: string; created_at: string; ended_at: string | null; cycles_committed: number; orchestrator_model: string; orchestrator: OrchestratorIdentity; consolidation: Consolidation | null; framework_computed: unknown; campaign_report_md: string | null; orchestration_log_md: string | null }
...
export interface DelegationCost { tokens: TokenTotals; model_calls: number; tool_calls: number; tool_failures: number; cache_hit_rate: number | null; wall_clock_minutes: number | null; cost_usd: number | null; cost_estimated: boolean }
...
export interface PlayoffPairing { order: number; delegation_id: string; source_experiment_id: string; incumbent_experiment_id: string; replayed_experiment_id: string | null; comparison_id: string | null; decision: string | null; decision_reason: string | null; mean_lift: number | null; challenger_win_rate: number | null; gates: Record<string, boolean>; guardrail_passed: boolean | null }
export interface Playoff { decision_mode: string; consolidation: Consolidation; finalists: PlayoffFinalist[]; exclusions: PlayoffExclusion[]; pairings: PlayoffPairing[]; final: PlayoffFinal | null; status: string; failure_reason: string | null; report_md: string | null }
...
export interface UsageByModel { key: string; provider: string; model: string; effort: string | null; role: string; tokens: TokenTotals | null; unmeasured: boolean; cost_usd: number | null; cost_estimated: boolean }
export interface TelemetrySummary { totals: TelemetryTotals; cache_hit_rate: number | null; by_delegation: TelemetryByDelegation[]; tool_mix: ToolMixEntry[]; usage_by_model: UsageByModel[]; cost_usd: number | null; cost_estimated: boolean }
