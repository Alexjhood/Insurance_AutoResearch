...
See [`docs/RUN_ORCHESTRATED.md`](docs/RUN_ORCHESTRATED.md).

An **orchestrator** conversation (an expensive model, small context) plans
directions and reads results; cheap headless **sub-agents** it spawns with
`autoresearch orchestrate spawn` run the experiment cycles inside their own
tracked runs. Each delegation ends with a framework-generated report; finalists
are consolidated head-to-head by `orchestrate playoff`.

```bash
cd <repo> && AUTORESEARCH_SCOPE=orchestrator claude
```

Then ask it to read [`ORCHESTRATOR.md`](ORCHESTRATOR.md). Single-agent mode is
unchanged: a sub-agent run is an ordinary tracked run following the same
`AGENT.md` contract.
...
│   ├── orchestrations/<orch-id>/       # campaign manifest, briefs, reports, playoff, log
...
Runs are kept independent. A research session is confined to its own run folder by a harness-level **run-scope guard** wired into all three agent harnesses (Claude Code, Codex, OpenCode): run artifacts are blocked before bootstrap, and once the session bootstraps it is bound to that run and cannot read any other run's files. Launch a deliberate cross-run analysis session with `AUTORESEARCH_SCOPE=analyst`, or an orchestrator session over one campaign's child runs with `AUTORESEARCH_SCOPE=orchestrator`. See [`docs/architecture.md`](docs/architecture.md) -> *Run-Scope Guard*.
...
- [`ORCHESTRATOR.md`](ORCHESTRATOR.md) — the orchestrator's contract for campaigns over sub-agents
- [`docs/RUN_ORCHESTRATED.md`](docs/RUN_ORCHESTRATED.md) — orchestrated-campaign quick start
